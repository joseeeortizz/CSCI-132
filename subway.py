...
def check_line(line):
    ...
    if not line[0] in ["BMT", "IND", "IRT"]: #Division
        ...
        return False
    if not re.match("^[a-zA-Z0-9\-\s]+$", line[1]): #Line
        print("Value: " + line[1] + " in line 1 is invalid")
        return False
    if not re.match("^[a-zA-Z0-9\-\s\(\)\&\']+$", line[2]): #Station Name
        print("Value: " + line[2] + " in line 2 is invalid")
        return False
    if not re.match("^\-?[0-9]+(\.?[0-9]+)?$", line[3]): #Station Latitude 
        print("Value: " + line[3] + " in line 3 is invalid")
        return False
    if not re.match("^\-?[0-9]+\.?[0-9]+?$", line[4]): #Station Longtitude
        print("Value: " + line[4] + " in line 4 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[5]): #Route 1
        print("Value: " + line[5] + " in line 5 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[6]): #Route 2
        print("Value: " + line[6] + " in line 6 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[7]): #Route 3
        print("Value: " + line[7] + " in line 7 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[8]): #Route 4
        print("Value: " + line[8] + " in line 8 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[9]): #Route 
        print("Value: " + line[9] + " in line 9 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[10]): #Route 6
        print("Value: " + line[10] + " in line 10 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[11]): #Route 7 
        print("Value: " + line[11] + " in line 11 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[12]): #Route 8
        print("Value: " + line[12] + " in line 12 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[13]): #Route 9
        print("Value: " + line[13] + " in line 13 is invalid")
        return False 
    if not re.match("[a-zA-Z0-9]|^$", line[14]): #Route 10
        print("Value: " + line[14] + " in line 14 is invalid")
        return False
    if not re.match("[a-zA-Z0-9]|^$", line[15]): #Route 11
    ...
    if not line[27] in ["NW", "NE", "SE", "SW","W","N","E","S","West",""]: #Corner
        ...
        return False
    if not re.match("^\-?[0-9]+\.?[0-9]+?$", line[28]): #Entrance Latidude
        print("Value: " + line[28] + " in line 28 is invalid")
        return False
    if not re.match("^\-?[0-9]+\.?[0-9]+?$", line[29]): #Entrace Longtitude
        print("Value: " + line[29] + " in line 29 is invalid")
        return False
    if not re.match("^\(\-?[0-9]+(\.?[0-9]+)?\,\s\-?[0-9]+(\.?[0-9]+)?\)$", line[30]): #Station Location
        print("Value: " + line[30] + " in line 30 is invalid")
        return False
    if not re.match("^\(\-?[0-9]+(\.?[0-9]+)?\,\s\-?[0-9]+(\.?[0-9]+)?\)$", line[31]): #Entrance Location
